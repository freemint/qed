...
short get_longestline(TEXTP t_ptr)
{
	...
	if (t_ptr->max_line != NULL)
	{
		...
		if (line->exp_len >= t_ptr->max_line->exp_len)	/* l„nger als die L„ngste */
		{
		...
		{
			if (line->is_longest)			/* wurde die L„ngste krzer */
	...
	if (t_ptr->max_line == NULL)			/* neu suche */
	{
		...
		while (!IS_TAIL(line))
		{
			if (line->exp_len == -1)		/* L„nge hat sich ge„ndert */
				line->exp_len = bild_pos(line->len, line, t_ptr->loc_opt->tab, t_ptr->loc_opt->tabsize) + 1;
			if (line->exp_len > len)
			{
				t_ptr->max_line = line;
			...
			line->is_longest = FALSE;		/* berall l”schen */
			NEXT(line);
		}
