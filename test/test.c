...
#include/*TestOhneTrennzeichen*/"xyz.h"
alabaster/*test*/(test);
#include <support.h>
#include <time.h>
...
/* Anzahl der énderungen in allen Texten bis zum restore_edit */
#define MAX_CHG 30
...
/*
 * Es wurde Zeilenumbruch ein oder ausgeschaltet und/oder Tab geÑndert 
*/
	...
	for (i = setmax(used_info); i >= min; i--)
	{
		if (setin(used_info, i))
		{
			...
			if (filematch(t_ptr->filename, mask, t_ptr->filesys))
			{
				switch (flag)
				{
					case 0 :							/* sichern ohne schlieûen */
						if (t_ptr->moved)			/* nur wenn nîtig! */
							do_icon(i, DO_SAVE);
						break;

					case 1 :							/* sichern und schlieûen */
						do_icon(i, DO_DELETE);
						break;

					case 2 :							/* schlieûen ohne sichern */
						t_ptr->moved = 0;
						do_icon(i, DO_DELETE);
						break;
...
/* Ermittelt aus einer Mauspos (mx,my) die zugehîrige Position im Text */
static void get_pos(WINDOWP window, int mx, int my, int *xpos, long *ypos)
{
	...
	if (font_prop)
	{
		...
		while (TRUE)
		{
			...
			if (i > x_soll)
				e = x;			/* in linker HÑlfte */
			else
				s = x;			/* in rechter HÑlfte */
			xl = x;				
		}
...
/*
 * PrÅft, ob Mauspos (x,y) innerhalb der Blockselektion liegt.
*/
...
abandon:	strcpy(name, t_ptr->filename);
		...
		case DO_ADD 	:
			...
			break;
		case DO_UPDATE	:
			/* Schreibschutz oder Datei verÑndert? */
...
static bool e_icon_drag(int icon, int source)
{
	...
	switch (source)
	{
		case DRAGDROP_FILE :						/* Inhalt von drag_filename einfÅgen */
			t_ptr = get_text(icon);
		...
		case DRAGDROP_PATH :						/* Text aus drag_filename einfÅgen */
			t_ptr = get_text(icon);
			if (drag_filename[0] != EOS && t_ptr != NULL)
			{
		...
		case DRAGDROP_DATA :						/* Textdaten einfÅgen */
...
/*
	SCROLL_UP 		Alles unter der aktuellen Zeile wird hochgescrollt,
						die letzte Zeile wird natÅrlich neu geschrieben.
	...
	BLK_CHANGE		Die Blockmarkierung wurde geÑndert.
	WT_CHANGE		Fenstertitel ('*') lîschen (nach save).
*/
	int i;

	if (change==TOTAL_CHANGE)
	{
		for (i=chg_anz; (--i)>=0; )			/* unnÅtzte énderung */
			if (chg[i].link==link)
			{
	...
	{
		for (i=chg_anz; (--i)>=0; )			/* gleiche énderung */
...
static void restore_offdesk(WINDOWP window, TCHANGE *c, int c_anz, TEXTP t_ptr)
{
	...
	for (; (--c_anz)>=0; c++)
	{
		switch (c->c)
		{
			case WT_CHANGE :	/* fÅr save(): nur '*' lîschen, sonst nix */
...
static void restore_indesk(WINDOWP window, TCHANGE *c, int c_anz, TEXTP t_ptr)
{
	...
	for (; (--c_anz)>=0; c++)
	{
		...
		switch (c->c)
		{
			case WT_CHANGE:				/* nur '*' Ñndern, sonst nix */
				change_window(window, t_ptr->filename, (t_ptr->moved!=0));
				break;
...
void restore_edit(void)
{
	...
	show_mouse();
	chg_anz = 0;			/* Keine énderungen mehr */
	setclr(chg_links);
}
...
bool info_edit (int icon)
{
	...
	if (t_ptr->text.ending != binmode)
		...
		set_string(textinfo, INFZEILE, "--");		/* BinÑr: keine Zeilen */
	...
	/* Dateien aus einem Projekt oder BinÑr -> kein Zeilenende */
